...
def answer_query(user_message: str) -> ChatResponse:
    ...
    research = get_stock_research(ticker, use_transformer=False)
    if research is None:
        return ChatResponse(
    ...
    text = (
        ...
        f"SMA20: {technicals.sma_20:.2f}, RSI14: {technicals.rsi_14:.2f}. "
        f"News sentiment: {sentiment.label} ({sentiment.score:.3f}, {sentiment.source}).{source_note}"
    )
